const buildStrandPipeline = ({ query = {}, projection = false }) => {
  const pipeline = [];
...
const buildStrandsPipeline = ({
  ...
  if (keyword && keyword.trim().length > 0) {
    ...
    // Dynamic search conditions for strand fields
    const strandSearchConditions = [{ title: { $regex: keywordRegex } }];

    pipeline.push({
      $match: {
        $or: strandSearchConditions,
      },
    });
  }
...
const buildStrandCountPipeline = ({ keyword, query = {} }) => {
  const pipeline = [];
